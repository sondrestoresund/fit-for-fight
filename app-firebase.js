...
// Public profiles
function recognizedRowForName(name){ try{ return window.buildModel?.().find(r=>r.name===name); }catch{ return null } }
...
function normalizeFsLogs(){ const norm=[]; (FS.logs||[]).forEach(l=>{ const row=recognizedRowForName(l.exercise); if(!row) return; let value=null; if(row.unit==='reps') value = l.reps ?? null; else if(row.unit==='sec') value = l.timeSec ?? null; else { value=null; } if(value==null || isNaN(value)) return; const tp = window.tierAndPoints5? window.tierAndPoints5(row.dir,value,row.prospect,row.std,row.strong,row.elite,row.max):{tier:'-',pts:0}; norm.push({date:new Date(l.date?.toDate?l.date.toDate():l.date).toISOString().slice(0,10),exercise:(window.exerciseKey?window.exerciseKey(row):row.name),label:`${row.cat} — ${row.name}`,value,unit:row.unit,dir:row.dir,tier:tp.tier,pts:tp.pts,source:'fs'}); }); window.FS_LOGS = norm; }

function renderFs(){ const rows=(window.buildModel?.()||[]); const fsExercises=document.getElementById('fsExercises'); if(fsExercises){ fsExercises.innerHTML = rows.map(r=>`<option value="${r.name}">`).join(''); }
  ...
  normalizeFsLogs(); try{ window.renderBenchmarks?.(); window.renderHome?.(); window.renderProgress?.(); }catch{}
}

async function refreshFs(){ const user=auth.currentUser; if(!user) return; FS.days = await listTestDays(user.uid); FS.logs = await listAllLogs(user.uid); try{ const fr = await listFriends(user.uid); let state=window.loadUserData?.(user.uid)||{}; state.fireteam=state.fireteam||{}; state.fireteam.friends=fr; window.saveUserData?.(user.uid,state); }catch{} renderFs(); try{ await publishPublicSummary(); }catch{} try{ setupSquadRealtime(); }catch{} }
